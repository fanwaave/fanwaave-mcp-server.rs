...
impl McpProcess {
    fn start() -> Self {
        let mut command = Command::new(env!("CARGO_BIN_EXE_fanwaave-mcp-server"));
        command
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null());
        ...
        let mut child = command.spawn().expect("spawn fanwaave MCP process");
...
fn stdio_wire_exposes_org_specific_client_and_provider_parity() {
    ...
    let initialized = mcp.request(
        ...
        &json!({
            ...
            "capabilities": {},
            "clientInfo": {"name": "fanwaave-parity-test", "version": "1.0.0"},
        }),
    );
    assert_eq!(
    ...
    assert!(resources.iter().all(|resource| resource["uri"]
        .as_str()
        .is_some_and(|uri| uri.contains("fanwaave"))));
    ...
    assert_eq!(identity["organization"], "fanwaave");
    assert_eq!(identity["repository"], "fanwaave/fanwaave-mcp-server.rs");
    ...
    assert_eq!(posture["organization"], "fanwaave");
