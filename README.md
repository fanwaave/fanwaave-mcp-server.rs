...
The exact `fanwaave/fanwaave-mcp-server.rs` organization, repository, service,
...
- `github_posture` — the exact Fanwaave repository and CI posture;
...
- `supabase_posture` — the configured Fanwaave Supabase health boundary;
- `neon_posture` — exact Neon organization/project and branch metadata;
- `cloudflare_posture` — the configured Fanwaave zone and DNS posture;
- `k8s_posture` — the exact Fanwaave namespace in the
  `ORESoftware/k8s-cluster` deployment plane;
- `nats_posture` — the Fanwaave service and dependency subjects; and
...
local stdio process accepts no bearer token. The `fanwaave-mcp-http` binary
...
public resource as audience, requires the Fanwaave project realm, AAL2,
`mcp:read` plus `fanwaave:inspect`, and a `fanwaave_viewer` or
`fanwaave_operator` role. It never forwards a caller token upstream. Exact
...
- Kubernetes: `ORE_MCP_K8S_ENABLED=1`, optionally
  `ORE_MCP_K8S_NAMESPACE` (otherwise the exact `fanwaave` namespace); and
- NATS: `ORE_MCP_NATS_URL`, scoped to
  `fanwaave.mcp.service.read.v1` and `fanwaave.mcp.dependencies.read.v1`.
